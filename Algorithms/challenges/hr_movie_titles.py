...
def getMovieTitles(substr):
    ...
    n_pages = int(requests.get(url).json()['total_pages'])
    for i in range(1, n_pages+1):
        url = "https://jsonmock.hackerrank.com/api/movies/search/?Title=" + substr + "&page=" + str(i)
        data_all = requests.get(url).json()['data']
        titles += [data['Title'] for data in data_all]
