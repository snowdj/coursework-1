...
def findEvents(symbols, startday,endday, marketSymbol,verbose=False):
	...
	# Completing the Data - Removing the NaN values from the Matrix
        #---!!! Note: filling NaN could result in more events !!!
	...
	for symbol in symbols:
	    for i in range(1,len(close[symbol])):
	        if close[symbol][i-1] >= 7.0 and close[symbol][i] < 7.0 : # When market fall is more than 3% and also the stock compared to market is also fell by more than 5%.
